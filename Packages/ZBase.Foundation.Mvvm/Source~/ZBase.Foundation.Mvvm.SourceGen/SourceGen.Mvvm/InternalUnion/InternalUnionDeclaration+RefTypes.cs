...
namespace ZBase.Foundation.Mvvm.InternalUnionSourceGen
{
    partial class InternalUnionDeclaration
    {
        ...
        )
        {
            foreach (var typeRef in RefTypeRefs)
            {
                try
                {
                    ...
                    var source = GetSourceForInternalUnionForRefType(typeRef, compilation.Assembly.Name);
        ...
        private static string GetSourceForInternalUnionForRefType(TypeRef typeRef, string assemblyName)
        {
            ...
            p.OpenScope();
            {
                ...
                p.OpenScope();
                {
                    ...
                    p.OpenScope();
                    {
                        ...
                        p = WirteImplicitConversions(typeRef, typeName, internalUnionName, unionName, p);
            ...
            static Printer WirteImplicitConversions(TypeRef typeRef, string typeName, string internalUnionName, string unionName, Printer p)
            {
                if (typeRef.Symbol.TypeKind != TypeKind.Interface)
                {
