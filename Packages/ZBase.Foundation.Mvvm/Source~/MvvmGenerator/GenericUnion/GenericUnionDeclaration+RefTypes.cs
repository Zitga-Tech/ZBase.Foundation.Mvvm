...
namespace ZBase.Foundation.Mvvm.GenericUnionSourceGen
{
    partial class GenericUnionDeclaration
    {
        ...
        {
            foreach (var structRef in RefTypeRefs)
            {
                try
                {
                    var syntax = structRef.Syntax;
                    var syntaxTree = syntax.SyntaxTree;
                    var source = WriteGenericUnionForRefType(structRef);
        ...
        private static string WriteGenericUnionForRefType(StructRef structRef)
        {
            ...
            p.OpenScope();
            {
                ...
                p = WirteImplicitConversions(structRef, typeName, structName, unionName, p);
            ...
            static Printer WirteImplicitConversions(StructRef structRef, string typeName, string structName, string unionName, Printer p)
            {
                if (structRef.TypeArgument.TypeKind != TypeKind.Interface)
                {
